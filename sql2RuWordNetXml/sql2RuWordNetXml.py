    ...
    with conn.cursor(cursor_factory=extras.RealDictCursor) as cur, open(filename, "w", encoding="utf-8") as file:
        ...
        for row in cur:
            ...
            if cid not in concepts:
                concept = {"id": cid, "name": row["c_name"], "gloss": row["gloss"], "relations": [], "entries": []}
...
def generate_lex_file(pos):
    ...
    types = all_types[pos]
    lex_file = {"N": "noun.all", "V": "verb.all", "Adj": "adj.all"}[pos]
    ...
    with conn.cursor(cursor_factory=extras.RealDictCursor) as cur, open(filename, "w", encoding="utf-8") as file:
        ...
        for cid, concept in concepts.items():
            ...
            for key, type_set in all_types.items():
                if key == pos:
                    continue
                for entry in concept["all_entries"]:
                    if entry["synt_type"] in type_set:
                        pointers.append(entry["name"] + ",+")
                        break
